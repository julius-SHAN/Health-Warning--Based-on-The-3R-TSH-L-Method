clc;clear;close all
% 作用：将提取的S特征输入GS-SVM中进行参数寻优和分类识别
%% 载入数据和参数初始化；
...
for n=1:4
    ...
    trainfeat=zeros(len,len_Y_fr);
    testfeat=zeros(len,len_Y_fr);
    for i=1:len
...
%         sig=mapminmax(sig,0,1);
    ...
    %% 保存参数寻优图
    path='..\model\GS-SVM\';
    pngname=[path,'M_GS_mitdb2_wt_fir_3R_sb16-c',...
    ...
    filename1=[path,'M_GS_mitdb2_wt_fir_3R_sb16-c',...
        num2str(c),'-g',num2str(g),'_',num2str(n),'.mat'];
    save(filename1,'Conmat','testtime','ACC')
    filename2=[path,'M_GS_mitdb2_wt_fir_3R_sb16-c',...
...
if n==4
    filename3=[path,'M_GS_mitdb2_wt_fir_3R_sb16_params_acc','.mat'];
