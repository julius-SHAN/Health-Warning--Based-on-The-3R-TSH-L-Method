clc;clear;close all
% 作用：将提取的TH特征输入GS-SVM中进行参数寻优和分类识别
%% 载入数据和参数初始化；
...
%         sig=mapminmax(sig,0,1);
    ...
    %% 保存参数寻优图
    path='..\model\GS-SVM\';
    pngname=[path,'M_GS_mitdb2_wt_fir_3R_sta2_t2_harm_stft90-c',...
    ...
    filename1=[path,'M_GS_mitdb2_wt_fir_3R_sta2_t2_harm_stft90-c',...
        num2str(c),'-g',num2str(g),'_',num2str(n),'.mat'];
    save(filename1,'Conmat','testtime','ACC')
    filename2=[path,'M_GS_mitdb2_wt_fir_3R_sta2_t2_harm_stft90-c',...
...
if n==4
    filename3=[path,'M_GS_mitdb2_wt_fir_3R_sta2_t2_harm_stft90_params_acc','.mat'];
