clc;clear;close all
% 作用：将提取的T特征输入GS-SVM中进行参数寻优和分类识别
%% 载入数据和参数初始化；
...
for n=1:4
    ...
    Fs=360;
    trainfeat=zeros(len,4);
    testfeat=zeros(len,4);
    for i=1:len
    ...
    %% 保存参数寻优图
    path='..\model\GS-SVM\';
    pngname=[path,'M_GS_mitdb2_wt_fir_3R_sta2_t2-c',...
    ...
    filename1=[path,'M_GS_mitdb2_wt_fir_3R_sta2_t2-c',...
        num2str(c),'-g',num2str(g),'_',num2str(n),'.mat'];
    save(filename1,'Conmat','testtime','ACC')
    filename2=[path,'M_GS_mitdb2_wt_fir_3R_sta2_t2-c',...
...
if n==4
    filename3=[path,'M_GS_mitdb2_wt_fir_3R_sta2_t2_params_acc','.mat'];
