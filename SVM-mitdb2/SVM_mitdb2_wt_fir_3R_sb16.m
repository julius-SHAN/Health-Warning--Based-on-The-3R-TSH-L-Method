...
for n=1:4
    ...
    matname=[matpath,'M_GS_mitdb2_wt_fir_3R_sb16_params_acc','.mat'];
    load(matname);
    fprintf('Finished!\n');
    fprintf('=============================================================\n');
    if n==1
    ...
    trainfeat=zeros(len_tr,len_Y_fr);
    testfeat=zeros(len_te,len_Y_fr);
    for i=1:len_tr
...
%         sig=mapminmax(sig,0,1);
        ...
        trainfeat(i,:)=mapminmax(Y_uniband,0,1);
    end
    %测试集特征
    for i=1:len_te
...
%         sig=mapminmax(sig,0,1);
        ...
        testfeat(i,:)=mapminmax(Y_uniband,0,1);
    end
    %清除多余数据
    ...
    path='..\model\SVM\';
    filename1=[path,'M_mitdb2_wt_fir_3R_sb16-c',...
        num2str(c),'-g',num2str(g),'_',num2str(n),'.mat'];
    save(filename1,'model','Conmat','testtime','ACC')
    filename2=[path,'M_mitdb2_wt_fir_3R_sb16-c',...
...
if n==4
    filename3=[path,'M_mitdb2_wt_fir_3R_sb16_params_acc','.mat'];
