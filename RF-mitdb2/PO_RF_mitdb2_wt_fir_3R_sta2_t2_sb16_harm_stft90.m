...
%     RFLeaf=[1,2,3,4,5,10,20,50,100,200,500];
    ...
    xlabel('决策树的数目');
    ylabel('均方误差(MSE)') ;
    title('袋外误差(OOB Error)');
    LeafTreelgd=legend({'1' '10' '20' '50' '100' '200' '500'},'Location','NorthEast');
    title(LeafTreelgd,'最小叶节点数');
    hold off;
%     path='..\model\PO-RF\';
%     filename1=[path,'M_PO_mitdb2_wt_fir_3R_sta2_t2_sb16_harm_stft90_',num2str(n),',opt.mat'];
%     save(filename1,'ACC')
%     pngname=[path,'M_PO_mitdb2_wt_fir_3R_sta2_t2_sb16_harm_stft90_',num2str(n),',opt.png'];
%     frame=getframe(gcf);
%     im=frame2im(frame);
%     imwrite(im,pngname);
    toc(st);
