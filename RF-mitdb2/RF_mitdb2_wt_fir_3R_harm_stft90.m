...
for n=1:4
    ...
    trainfeat=zeros(len_tr,90);
    testfeat=zeros(len_te,90);
    for i=1:len_tr
...
%         sig=mapminmax(sig,0,1);
    ...
    path='..\model\RF\';
    filename1=[path,'M_mitdb2_wt_fir_3R_harm_stft90_t',num2str(ntree),...
        '_',num2str(nleaf),'_',num2str(n),'.mat'];
    save(filename1,'model','Conmat','testtime','ACC')
    filename2=[path,'M_mitdb2_wt_fir_3R_harm_stft90_t',num2str(ntree),...
        '_',num2str(nleaf),'_',num2str(n),'.png'];
    frame=getframe(gcf);
    im=frame2im(frame);
...
if n==4
    filename3=[path,'M_mitdb2_wt_fir_3R_harm_stft90_params_acc','.mat'];
