...
for n=1:4
    ...
    hold off;
    path='..\model\PO-RF\';
    filename1=[path,'M_PO_mitdb2_wt_fir_3R_sta2_t2_',num2str(n),',opt.mat'];
    save(filename1,'ACC')
    pngname=[path,'M_PO_mitdb2_wt_fir_3R_sta2_t2_',num2str(n),',opt.png'];
    frame=getframe(gcf);
    im=frame2im(frame);
    imwrite(im,pngname);
    toc(st);
