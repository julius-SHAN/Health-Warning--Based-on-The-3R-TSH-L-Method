...
for n=1:4
    ...
    Traindata=Traindata(num1(1:len1),:);
    Labeltrain=Labeltrain(num1(1:len1));
    RR_tr=RR_tr(num1(1:len1),:);
    Testdata=Testdata(num2(1:len2),:);
    Labeltest=Labeltest(num2(1:len2));
    RR_te=RR_te(num2(1:len2),:);
    ...
    for i=1:len_tr
        ...
        for m=1:len_Y_fr
            Ycount(m+1)=round((Y_fr(m))/deltaf)+1;
            temp=sqrt(sum(Y(Ycount(m):Ycount(m+1)).^2));
            Y_uniband(m)=log10(temp);
        end
...
%     confusionchart(Labeltest,ptest,'RowSummary','row-normalized','ColumnSummary','column-normalized');
    ...
    path='..\model\KNN\';
    filename1=[path,'M_mitdb2_wt_fir_3R_sta2_t2_sb16_harm_stft90_K',num2str(k),...
        '_D',d,'_',num2str(n),'.mat'];
    save(filename1,'model','Conmat','testtime','ACC')
%     filename2=[path,'M_mitdb2_wt_fir_3R_sta2_t2_sb16_harm_stft90_K',num2str(k),...
